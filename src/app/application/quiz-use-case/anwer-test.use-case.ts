...
export class AnswerTestUseCase {
  ...
  answerTest(token:string, questionSentence:string, answer:string[]) :Observable<TestModel> {
    return this.factoryApiService.createApiTest().answerTest(token, questionSentence, answer);
  }
}